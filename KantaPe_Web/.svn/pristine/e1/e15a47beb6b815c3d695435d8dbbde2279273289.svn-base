...
function CargarLocal(bln_pAsync, int_pIdLocal) {
    ...
    if (int_IdPersona != '') {
        ...
        AccionDefault(bln_pAsync, "RegistroOferta.aspx/ListarLocal", obj_Data,
             function (obj_pLista) {
...
function CargarPromocion(obj_pFuncion) {
    var int_IdLocal = $("#ddlLocal").val();
    if (int_IdLocal != '') {
        AccionDefault(true, "RegistroOferta.aspx/ObtenerLocal", { "int_pIdLocal": int_IdLocal },
             function (obj_pLocal) {
                 $("#chkPromocionar").prop('disabled', false);

                 if (obj_pFuncion != null)
                     obj_pFuncion.apply(this, null);

                 $("#lblPromocionar").html("Promocionar (" + obj_pLocal.Promocion + " Disponible(s))");
                 if (obj_pLocal.Promocion <= 0)
                     $("#chkPromocionar").prop('disabled', true);
        ...
        $("#lblPromocionar").html("Promocionar (0 Disponible)");
        $("#chkPromocionar").prop('disabled', true);
    }
}
function GuardarOferta() {
    ...
    if ($('#frmContent').valid()) {
        ...
        obj_Oferta.FlagPromocion = ($("#chkPromocionar").prop('checked') ? 'S' : 'N');
        ...
        if ($("#chkPromocionar").prop('checked') && !$("#chkPromocionar").prop('disabled')) {
            ConfirmJQ("¿Está seguro de promocionar la Oferta?", function () {
                Accion('RegistroOferta.aspx/GuardarOferta', { "str_pOferta": JSON.stringify(obj_Oferta) }, function () {
                    document.location.href = urlPrincipal;
                });
            }, null, null, null);
        }
...
function ObtenerOferta() {
    ...
    if (int_IdOferta != '') {
        ...
        if (obj_Oferta != null) {
            ...
            CargarLocal(false, obj_Oferta.IdLocal);
            ...
            //Foto
            $("#lnkEliminarFoto").css('display', '');
            if (obj_Oferta.ImagenURL == '') {
                obj_Oferta.ImagenURL = $("#hdOfertaImagen").val();
                $("#lnkEliminarFoto").css('display', 'none');
            }
            $("#imgOfertaImagen").attr("src", obj_Oferta.ImagenURL);

            //Promocion
            CargarPromocion(function () {
                $("#txtFechaPromocion").val(ToDateTimeString(obj_Oferta.FechaPromocion));
                $("#chkPromocionar").prop('checked', (obj_Oferta.FlagPromocion == 'S'));
                $("#chkPromocionar").prop('disabled', $("#chkPromocionar").prop('checked'));
            });

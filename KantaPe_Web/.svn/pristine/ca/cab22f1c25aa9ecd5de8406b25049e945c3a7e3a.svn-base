create PROCEDURE [dbo].[ReporteUsuarioFrecuenteComparacion] 
/*
'**********************************************************************************
...
BEGIN
		...
		From(
			Select
				ROW_NUMBER() OVER (ORDER BY  US.ApellidoPaterno + ' '+ US.ApellidoMaterno+', '+ US.Nombres asc) AS Num_Fila,
			    CONVERT(DATE,AU.FechaCreacion) as Fecha,
				--Empresa
			...
			US.ApellidoMaterno like '%' + @pNombreCompleto + '%' 
				...
				CONVERT(DATE,AU.FechaCreacion) between CONVERT (DATE, @pFechaInicio) and CONVERT (DATE, @pFechaFin)
				 ...
				 group by CONVERT(DATE,AU.FechaCreacion), US.IdPersona,EM.NombreComercial,CPD.Direccion, US.ApellidoPaterno + ' '+ US.ApellidoMaterno+', '+ US.Nombres, CPM.Mail

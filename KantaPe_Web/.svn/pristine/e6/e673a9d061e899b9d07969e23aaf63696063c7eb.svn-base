<%@ WebHandler Language="C#" Class="ReporteUsuarioFrecuenteExcel" %>
...
public class ReporteUsuarioFrecuenteExcel : IHttpHandler
{
    #region Propiedades
    ...
    {
        System.Collections.Generic.List<Kruma.KantaPe.Entidad.AperturaUsuario> lst_Lista = ReporteUsuarioFrecuente().Result;

        System.Text.StringBuilder stb_Reporte = new System.Text.StringBuilder();
        stb_Reporte.Append("<table>");
        stb_Reporte.Append("<tr><td colspan='2' style='font-size:18px; font-weight:bold;'>Reporte de usuario frecuente</td><td></td></tr>");
        ...
        stb_Reporte.Append("<th>Usuario</th>");
        stb_Reporte.Append("<th>Correo</th>");
        stb_Reporte.Append("<th>Empresa</th>");
        stb_Reporte.Append("<th>Local</th>");
        stb_Reporte.Append("<th>Frecuencia</th>");
        ...
        if (lst_Lista.Count > 0)
        {
            for (int i = 0; i < lst_Lista.Count; i++)
            {
                ...
                stb_Reporte.AppendFormat("<td>{0}</td>", lst_Lista[i].Persona.Mail+ "</td>");
                stb_Reporte.AppendFormat("<td>{0}</td>", lst_Lista[i].Empresa.NombreComercial+ "</td>");
                stb_Reporte.AppendFormat("<td>{0}</td>", lst_Lista[i].Local.Direccion+ "</td>");
                stb_Reporte.AppendFormat("<td>{0}</td>", lst_Lista[i].Cantidad+ "</td>");
        ...
        obj_pContext.Response.AddHeader("content-disposition", "attachment; filename=ReporteUsuarioFrecuente_" + date + ".xls");
    ...
    private Kruma.Core.Util.Common.List<Kruma.KantaPe.Entidad.AperturaUsuario> ReporteUsuarioFrecuente()
    {
        return Kruma.KantaPe.Negocio.AperturaUsuario.lstReporteUsuarioFrecuente;
    }

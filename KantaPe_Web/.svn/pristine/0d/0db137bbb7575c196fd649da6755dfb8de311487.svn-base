...
function ListarEmpleado(int_pNumPagina) {
    ...
    AccionDefault(true, "Evento.aspx/ListarEmpleado", obj_Data, CargarGrilla, null, null, null, 1);
}
...
function CargarGrilla(obj_pLista) {
    ...
    obj_FilaPrincipal.append($("<th style='width:20%;'>Tipo de Documento</th>"));
    obj_FilaPrincipal.append($("<th style='width:15%;'>N° Documento</th>"));
    ...
    for (var i = 0; i < obj_pLista.Result.length; i++) {
        ...
        obj_FilaPrincipal.append($("<td>" + obj_pLista.Result[i].Persona.TipoDocmento.Descripcion + "</td>"));
...
function ModificarEstado(obj_pKey, str_pEstado) {
    Accion('Empleado.aspx/ModificarEstado', {

...
function ListarGenero(int_pNumPagina) {
    ...
    AccionDefault(true, "Genero.aspx/ListarGenero", obj_Data, CargarGrilla, null, null, null, 1);
}

function ListarGeneroResultado(obj_pResultado, int_pNumPagina) {
    ListarGenero(int_pNumPagina);
}

function CargarGrilla(obj_pLista) {
    var obj_TablaPrincipal = $("<table id='tblGenero' class='table table-bordered table-hover dt-responsive'></table>");
    ...
    for (var i = 0; i < obj_pLista.Result.length; i++) {
        obj_FilaPrincipal = $("<tr value='" + obj_pLista.Result[i].IdGenero + "'></tr>");
    ...
    var obj_Tabla = $('#tblGenero').dataTable({
        "language": {
    ...
    $('#tblGenero tbody').on('click', 'tr', function () {
...
function obtenerIdRegistro() {
    var str_Id = null;
    var obj_Seleccion = $('#tblGenero').find('.selected');
...
function Activar() {
    var str_Id = obtenerIdRegistro();
    if (str_Id != null) {
        ConfirmJQ('¿Está seguro de activar el género?', ModificarEstado, [str_Id, 'A']);
    }
}
...
function Inactivar(int_pId) {
    var str_Id = obtenerIdRegistro();
    if (str_Id != null) {
        ConfirmJQ('¿Está seguro de inactivar el género?', ModificarEstado, [str_Id, 'I']);
    }
}

function ModificarEstado(str_pId, str_pEstado) {
    Accion('Genero.aspx/ModificarEstado', { "int_pIdGenero": str_pId, "str_pEstado": str_pEstado }, ListarGeneroResultado, [1]);
}
create PROCEDURE [dbo].[ListarEvento]
/*
'**********************************************************************************
...
Begin
		...
		ROW_NUMBER() OVER (ORDER BY T.Nombre ASC) AS Num_Fila,
			...
			(@pIdEvento is null or T.IdEvento = @pIdEvento) AND
			(@pNombre is null or T.Nombre like '%' + @pNombre + '%') AND 
			(@pDescripcion is null or T.Descripcion like '%' + @pDescripcion + '%') AND 
			(@pFechaInicio is null or (T.FechaEvento is not null AND
			(convert(datetime,convert(varchar(8),T.FechaEvento,112)) >= convert(datetime,convert(varchar(8),@pFechaInicio,112))))) AND
			(@pFechaFin is null or (T.FechaEvento is not null AND
			(convert(datetime,convert(varchar(8),T.FechaEvento,112)) <= convert(datetime,convert(varchar(8),@pFechaFin,112))))) AND

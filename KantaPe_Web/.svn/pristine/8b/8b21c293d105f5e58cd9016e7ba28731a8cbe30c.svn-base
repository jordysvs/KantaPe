<%@ WebHandler Language="C#" Class="ReporteAperturaMesaExcel" %>
...
public class ReporteAperturaMesaExcel : IHttpHandler
{
    #region Propiedades
    ...
    {
        System.Collections.Generic.List<Kruma.KantaPe.Entidad.Apertura> lst_Lista = ReporteAperturaMesa().Result;

        System.Text.StringBuilder stb_Reporte = new System.Text.StringBuilder();
        stb_Reporte.Append("<table>");
        stb_Reporte.Append("<tr><td colspan='2' style='font-size:18px; font-weight:bold;'>Reporte de Aperura de mesa</td><td></td></tr>");
        ...
        stb_Reporte.Append("<th>Tipo de Ubicación</th>");
        stb_Reporte.Append("<th>Ubicacion</th>");
        stb_Reporte.Append("<th>Promedio Atención (min)</th>");
        ...
        if (lst_Lista.Count > 0)
        {
            for (int i = 0; i < lst_Lista.Count; i++)
            {
                ...
                stb_Reporte.AppendFormat("<td>{0}</td>", str_UbicacionTipo+ "</td>");
                stb_Reporte.AppendFormat("<td>{0}</td>", str_Numero+ "</td>");
        ...
        obj_pContext.Response.AddHeader("content-disposition", "attachment; filename=ReporteAperturaMesa_" + date + ".xls");
    ...
    private Kruma.Core.Util.Common.List<Kruma.KantaPe.Entidad.Apertura> ReporteAperturaMesa()
    {
        return Kruma.KantaPe.Negocio.Apertura.lstReporteAperturaMesa;
    }

...
function ObtenerFiltros() {
    ...
    obj_Filtro.Ip = $("#txtIp").val();
    obj_Filtro.Nombre = $("#txtNombre").val();
    obj_Filtro.Estado = $("#ddlEstado").val();
    return obj_Filtro;
}

function ListarDispositivoResultado(obj_pResultado, int_pNumPagina) {
    ListarDispositivo(int_pNumPagina);
}

function ListarDispositivo(int_pNumPagina) {
    int_NumPagina = int_pNumPagina;
    var obj_Filtro = ObtenerFiltros();
    var obj_Data = { "str_pIp": obj_Filtro.Ip, "str_pNombre": obj_Filtro.Nombre, "str_pEstado": obj_Filtro.Estado, "int_pNumPagina": int_pNumPagina, "int_pTamPagina": int_TamPagina };
    AccionDefault(true, "Dispositivo.aspx/ListarDispositivo", obj_Data, CargarGrilla, null, null, null, 1);
}

function CargarGrilla(obj_pLista) {

    var obj_TablaPrincipal = $("<table id='tblDispositivo' class='table table-bordered table-hover dt-responsive'></table>");
    var obj_THead = $("<thead></thead>");
    var obj_FilaPrincipal = $("<tr></tr>");
    obj_FilaPrincipal.append($("<th>Ip</th>"));
    obj_FilaPrincipal.append($("<th>Host Name</th>"));
    ...
    for (var i = 0; i < obj_pLista.Result.length; i++) {
        obj_FilaPrincipal = $("<tr value='" + obj_pLista.Result[i].IdDispositivo + "'></tr>");
        obj_FilaPrincipal.append($("<td style='width:20%'>" + obj_pLista.Result[i].Ip + "</td>"));
        obj_FilaPrincipal.append($("<td style='width:65%'>" + obj_pLista.Result[i].Nombre + "</td>"));
    ...
    $("#divPaginacion").bootpag({
        ...
        ListarDispositivo(num);
    });

    $("#divGrilla").html('');
    $("#divGrilla").append(obj_TablaPrincipal);

    var obj_Tabla = $('#tblDispositivo').dataTable({
        "language": {
    ...
    $('#tblDispositivo tbody').on('click', 'tr', function () {
...
function obtenerIdRegistro() {
    var str_Id = null;
    var obj_Seleccion = $('#tblDispositivo').find('.selected');
    if (obj_Seleccion.length > 0)
        str_Id = $(obj_Seleccion[0]).attr('value');
    else
        AlertJQ(1, 'Seleccione un dispositivo.');
    return str_Id;
}
...
function Activar() {
    var str_Id = obtenerIdRegistro();
    if (str_Id != null) {
        ConfirmJQ('¿Está seguro de activar el dispositivo?', ModificarEstado, [str_Id, 'A']);
    }
}
...
function Inactivar(int_pId) {
    var str_Id = obtenerIdRegistro();
    if (str_Id != null) {
        ConfirmJQ('¿Está seguro de inactivar el dispositivo?', ModificarEstado, [str_Id, 'I']);
    }
}

function ModificarEstado(str_pId, str_pEstado) {
    Accion('Dispositivo.aspx/ModificarEstado', { "int_pIdDispositivo": str_pId, "str_pEstado": str_pEstado }, ListarDispositivoResultado, [1]);
}

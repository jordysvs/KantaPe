...
function ObtenerFiltros() {
    ...
    obj_Filtro.IdProductoTipo = null;
    if ($("#ddlProductoTipo").val() != '')
        obj_Filtro.IdProductoTipo = $("#ddlProductoTipo").val();
...
function CargarLocal(bln_pAsync, int_pIdLocal) {
    ...
    if (int_IdPersona != '') {
        ...
        AccionDefault(bln_pAsync, "Producto.aspx/ListarLocal", obj_Data,
             function (obj_pLista) {
...
function ListarProducto(int_pNumPagina) {
    ...
    var obj_Data = {
        ...
        "int_pIdProductoTipo": obj_Filtro.IdProductoTipo,
        "str_pEstado": obj_Filtro.Estado,
        "int_pNumPagina": int_pNumPagina,
        "int_pTamPagina": int_TamPagina
    };
    AccionDefault(true, "Producto.aspx/ListarProducto", obj_Data, CargarGrilla, null, null, null, 1);
}

function CargarGrilla(obj_pLista) {

    var obj_TablaPrincipal = $("<table id='tblProducto' class='table table-bordered table-hover dt-responsive'></table>");
    ...
    obj_FilaPrincipal.append($("<th style='width:25%;'>Nombre</th>"));
    ...
    for (var i = 0; i < obj_pLista.Result.length; i++) {
        ...
        obj_FilaPrincipal = $("<tr idlocal='" + obj_pLista.Result[i].IdLocal + "' idproducto='" + obj_pLista.Result[i].IdProducto + "'></tr>");
        ...
        obj_FilaPrincipal.append($("<td>" + obj_pLista.Result[i].ProductoTipo.Descripcion + "</td>"));
    ...
    var obj_Tabla = $('#tblProducto').dataTable({
        "language": {
...
function ObtenerIdRegistro() {
    var obj_Key = [];
    var obj_Seleccion = $('#tblProducto').find('.selected');
    if (obj_Seleccion.length > 0) {
        obj_Key.push($(obj_Seleccion[0]).attr('idlocal'));
        obj_Key.push($(obj_Seleccion[0]).attr('idproducto'));
    }
    else
        AlertJQ(1, 'Seleccione un producto.');
    return obj_Key;
}
...
function Activar() {
    var obj_Key = ObtenerIdRegistro();
    if (obj_Key.length > 0) {
        ConfirmJQ('¿Está seguro de activar el producto?', ModificarEstado, [obj_Key, 'A']);
    }
}
...
function Inactivar() {
    var obj_Key = ObtenerIdRegistro();
    if (obj_Key.length > 0) {
        ConfirmJQ('¿Está seguro de inactivar el producto?', ModificarEstado, [obj_Key, 'I']);
    }
}

function ModificarEstado(obj_pKey, str_pEstado) {
    Accion('Producto.aspx/ModificarEstado', {
        "int_pIdLocal": obj_pKey[0],
        "int_pIdProducto": obj_pKey[1],
        "str_pEstado": str_pEstado
    }, function () { ListarProducto(1); });
}
...
function CargarLocal(bln_pAsync, int_pIdLocal) {
    ...
    if (int_IdPersona != '') {
        ...
        AccionDefault(bln_pAsync, "Evento.aspx/ListarLocal", obj_Data,
             function (obj_pLista) {
...
function ListarEvento(int_pNumPagina) {
    ...
    AccionDefault(true, "Evento.aspx/ListarEvento", obj_Data, CargarGrilla, null, null, null, 1);
}

function CargarGrilla(obj_pLista) {

    var obj_TablaPrincipal = $("<table id='tblEvento' class='table table-bordered table-hover dt-responsive'></table>");
    ...
    obj_FilaPrincipal.append($("<th style='width:15%;'>Fecha Evento</th>"));
    ...
    for (var i = 0; i < obj_pLista.Result.length; i++) {
        ...
        obj_FilaPrincipal = $("<tr idlocal='" + obj_pLista.Result[i].IdLocal + "' idevento='" + obj_pLista.Result[i].IdEvento + "'></tr>");
        ...
        obj_FilaPrincipal.append($("<td>" + ToDateString(obj_pLista.Result[i].FechaEvento) + "</td>"));
    ...
    var obj_Tabla = $('#tblEvento').dataTable({
        "language": {
    ...
    $('#tblEvento tbody').on('click', 'tr', function () {
...
function ObtenerIdRegistro() {
    var obj_Key = [];
    var obj_Seleccion = $('#tblEvento').find('.selected');
    if (obj_Seleccion.length > 0) {
        obj_Key.push($(obj_Seleccion[0]).attr('idlocal'));
        obj_Key.push($(obj_Seleccion[0]).attr('idevento'));
    }
    else
        AlertJQ(1, 'Seleccione un evento.');
    return obj_Key;
}
...
function Activar() {
    var obj_Key = ObtenerIdRegistro();
    if (obj_Key.length > 0) {
        ConfirmJQ('¿Está seguro de activar el evento?', ModificarEstado, [obj_Key, 'A']);
    }
}
...
function Inactivar() {
    var obj_Key = ObtenerIdRegistro();
    if (obj_Key.length > 0) {
        ConfirmJQ('¿Está seguro de inactivar el evento?', ModificarEstado, [obj_Key, 'I']);
    }
}
...
function ModificarEstado(obj_pKey, str_pEstado) {
    Accion('Evento.aspx/ModificarEstado', {
        ...
        "int_pIdEvento": obj_pKey[1],
        "str_pEstado": str_pEstado
    }, function () { ListarEvento(1); });
}
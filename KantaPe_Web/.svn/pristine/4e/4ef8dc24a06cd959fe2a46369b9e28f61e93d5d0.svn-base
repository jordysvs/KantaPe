create PROCEDURE [dbo].[ListarMovilGenero]
/*
'**********************************************************************************
...
As
Begin
		INSERT INTO TblProcedure EXEC [JRG_AdmEscV1].[ListarMovilGenero]
		Select tbl.*
		from(
			Select
				...
				From Genero G
				Where 
				IdGenero in (select IdGenero from TblProcedure  where IdGenero is not null)
				--AND (@pIdGenero is null or G.IdGenero = @pIdGenero) 
				--AND (@pDescripcion is null or G.Descripcion like '%' + @pDescripcion + '%') 
				--AND  (@pEstado is null or G.Estado = @pEstado) 
		) tbl

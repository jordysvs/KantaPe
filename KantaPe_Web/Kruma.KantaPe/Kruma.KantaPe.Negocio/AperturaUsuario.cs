...
namespace Kruma.KantaPe.Negocio
{
    ...
    public class AperturaUsuario
    {
        ...
        public static ProcessResult Insertar(Kruma.KantaPe.Entidad.AperturaUsuario obj_pAperturaUsuario)
        {
            ...
            try
            {
                int int_IdAperturaUsuario = Kruma.KantaPe.Data.AperturaUsuario.Insertar(obj_pAperturaUsuario);
                ...
                obj_Resultado = new ProcessResult(int_IdAperturaUsuario);
            }
                ...
                using (TransactionScope obj_Transaction = new TransactionScope())
                {
                  ...
                  obj_pAperturaUsuario.IdUsuario);
                    ...
                    if (obj_pAperturaUsuario.Estado.Equals("I"))
                    {
                        ...
                        //Inactivar pedidos
                        System.Collections.Generic.List<Kruma.KantaPe.Entidad.AperturaUbicacion> lst_AperturaUbicacionPedidos = Kruma.KantaPe.Data.AperturaUbicacion.Listar(new int?(obj_pAperturaUsuario.IdLocal.Value), new int?(obj_pAperturaUsuario.IdApertura.Value), null, "A", null, null).Result;
                      
                            System.Collections.Generic.List<Kruma.KantaPe.Entidad.Alerta> lst_pSolicitudesAModificar = Kruma.KantaPe.Negocio.Alerta.ObtenerAltertaPedidos(
                       obj_pAperturaUsuario.IdLocal.Value,
                       lst_AperturaUbicacionPedidos[0].IdUbicacion.Value,
                       2,
                       obj_pAperturaUsuario.IdUsuario,
                       KantaPe.Entidad.Constante.Estado_Pendiente).Result;

                        foreach (Kruma.KantaPe.Entidad.Alerta obj_pSolicitudesAModificar in lst_pSolicitudesAModificar)
                        {
                            obj_pSolicitudesAModificar.Estado = KantaPe.Entidad.Constante.Estado_Inactivo;
                            Kruma.KantaPe.Data.Alerta.Modificar(obj_pSolicitudesAModificar);
                        }
                        //Final Incativar pedidos
                    }
                    obj_Resultado = new ProcessResult(obj_pAperturaUsuario.IdUsuario);
                    obj_Transaction.Complete();
                }

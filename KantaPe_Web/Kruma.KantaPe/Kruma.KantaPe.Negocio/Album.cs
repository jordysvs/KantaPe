    ...
    public class Album
    {
        ...
        public static Kruma.Core.Util.Common.ProcessResult Insertar(Kruma.KantaPe.Entidad.Album obj_pAlbum)
        {
            ...
            try
            {
                ...
                {
                    System.Collections.Generic.List<Kruma.KantaPe.Entidad.Album> lst_Registros = Kruma.KantaPe.Data.Album.Listar(null, null, null, null, null, null, null).Result;
                    if (lst_Registros.Where(x => x.Titulo.Trim().ToLower().Equals(obj_pAlbum.Titulo.Trim().ToLower())
                                                    || x.Titulo.Trim().ToUpper().Equals(obj_pAlbum.Titulo.Trim().ToUpper()))
                                                                    .ToList().Count > 0)
                        return new ProcessResult(new Exception("Ya existe un álbum con el mismo título."));
                    //Inserción del album

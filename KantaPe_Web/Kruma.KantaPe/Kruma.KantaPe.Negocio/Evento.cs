...
namespace Kruma.KantaPe.Negocio
{
    ...
    public class Evento
    {
        ...
        public static Kruma.Core.Util.Common.List<Kruma.KantaPe.Entidad.Evento> Listar(
            ...
            int? int_pTamPagina)
        {
            Kruma.Core.Util.Common.List<Kruma.KantaPe.Entidad.Evento> lst_Lista = Kruma.KantaPe.Data.Evento.Listar(
                int_pIdEmpresa,
                int_pIdLocal,
                int_pIdEvento,
                str_pNombre,
            ...
            foreach (Kruma.KantaPe.Entidad.Evento obj_Evento in lst_Lista.Result)
                if (obj_Evento.IdImagen != null)
                    obj_Evento.ImagenURL = ObtenerEventoImagenURL(obj_Evento.IdLocal, obj_Evento.IdImagen);
        ...
        public static Kruma.KantaPe.Entidad.Evento Obtener(
            ...
            int int_pIdEvento)
        {
            Kruma.KantaPe.Entidad.Evento obj_Evento = Kruma.KantaPe.Data.Evento.Obtener(int_pIdLocal, int_pIdEvento);
            if (obj_Evento.IdImagen != null)
                obj_Evento.ImagenURL = ObtenerEventoImagenURL(obj_Evento.IdLocal, obj_Evento.IdImagen);
            return obj_Evento;
        }
                    ...
                    if (obj_EventoInsertado.Foto != null)
                    {
                        if (obj_EventoInsertado.Foto.IdDocumento.HasValue)
                        {
                            ...
                            Kruma.Core.FileServer.Entity.FileUpload obj_Foto = new Kruma.Core.FileServer.FileServerManager().copyFile(
                                ...
                                obj_EventoInsertado.IdLocal.Value,
                                obj_EventoInsertado.UsuarioCreacion);

                            obj_EventoInsertado.IdImagen = obj_Foto.IdDocumento;
                            Kruma.KantaPe.Data.Evento.Modificar(obj_EventoInsertado);
                        }
                    }
            ...
            try
            {
                using (TransactionScope obj_Transaction = new TransactionScope())
                {
                    ...
                    if (obj_EventoAModificar.Foto != null)
                    {
                        ...
                        if (obj_EventoAModificar.IdImagen.HasValue)
                        {
                            Kruma.Core.FileServer.Entity.FileUpload obj_Foto = obj_FileServerManager.getFile(int_IdAlmacenDestino, obj_EventoAModificar.IdLocal.Value, obj_EventoAModificar.IdImagen.Value);
                            obj_FileServerManager.deleteFile(obj_Foto.IdDocumento.Value, obj_Foto.IdAlmacen.Value, obj_Foto.IdRegistro.Value, obj_EventoAModificar.UsuarioModificacion);
                        }
                        if (obj_EventoAModificar.Foto.IdDocumento.HasValue)
                        {
                            Kruma.Core.FileServer.Entity.FileUpload obj_Foto = obj_FileServerManager.copyFile(
                                ...
                                int_IdAlmacenDestino,
                                obj_EventoAModificar.IdLocal.Value,
                                obj_EventoAModificar.UsuarioModificacion);

                            obj_EventoAModificar.IdImagen = obj_Foto.IdDocumento;
                        }
                    }
        ...
        public static string ObtenerEventoImagenURL(int? int_pIdRegistro, int? int_pIdDocumento)
        {

...
namespace Kruma.KantaPe.Negocio
{
    ...
    public class Alerta
    {
        ...
        {
            return Kruma.KantaPe.Data.Alerta.ObtenerAltertaPedidos(int_pIdLocal, int_pIdUbicacion, int_pIdAlertaTipo, str_pIdUsuario, str_pEstado);
        }
        ...
        public static ProcessResult AlertarTurno(Kruma.KantaPe.Entidad.Alerta obj_pAlerta)
        {
            ...
            try
            {
                ...
                {
                    if (obj_pAlerta.IdAlertaTipo == Kruma.KantaPe.Entidad.Constante.AlertaTipo_Turno)
                        Notificacion.Enviar(obj_AperturaUsuario.IdUsuario, "Turno de mesa",
                            string.Format("Su turno comenzará en {0} segundos.", obj_TiempoTurno.Valor),
                            new { IdTipoNotificacion = Kruma.KantaPe.Entidad.Constante.Notificacion_PerfilCancion });
                    else if (obj_pAlerta.IdAlertaTipo == Kruma.KantaPe.Entidad.Constante.AlertaTipo_PerdidaTurno)
                        Notificacion.Enviar(obj_AperturaUsuario.IdUsuario, "Turno de mesa",
                            string.Format("Su turno comenzará en {0} segundos y todavía no solicita ninguna canción.", obj_TiempoTurno.Valor),
                            new { IdTipoNotificacion = Kruma.KantaPe.Entidad.Constante.Notificacion_Cancion });
                }
        ...
        /// </summary>
        /// <param name="obj_pAlerta">Alerta</param>
        public static void NotificarMozos(Entidad.Alerta obj_pAlerta)
        {
            ...
            if (obj_pAlerta.IdAlertaTipo.Value == Kruma.KantaPe.Entidad.Constante.AlertaTipo_Apertura)
            {
                ...
                int_TipoNotificacion = Kruma.KantaPe.Entidad.Constante.Notificacion_Apertura;
            }
            else if (obj_pAlerta.IdAlertaTipo.Value == Kruma.KantaPe.Entidad.Constante.AlertaTipo_Carta)
            {
                ...
                int_TipoNotificacion = Kruma.KantaPe.Entidad.Constante.Notificacion_SolicitudPerdido;
            }

            foreach (Kruma.KantaPe.Entidad.Usuario obj_Mozo in lst_Mozos)
            {
                Notificacion.Enviar(obj_Mozo.IdUsuario, str_Titulo, str_Mensaje, new { IdTipoNotificacion = int_TipoNotificacion });
            }
        }

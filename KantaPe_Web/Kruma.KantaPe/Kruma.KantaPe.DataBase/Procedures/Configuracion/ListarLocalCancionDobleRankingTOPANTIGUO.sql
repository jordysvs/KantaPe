create PROCEDURE [dbo].[ListarLocalCancionDobleRankingTOPANTIGUO]
/*
'**********************************************************************************
...
Begin
		...
		--CAST(LEFT(userID, 4) AS int)
			Select top (select Convert(int, rtrim(valor)) from [dbo].CoreParametro where IdParametro = 'DIASNOVEDADES')
			...
			(@pPalabraClave1 is null or 
					(CS.Descripcion like '%' + @pPalabraClave1 + '%') OR

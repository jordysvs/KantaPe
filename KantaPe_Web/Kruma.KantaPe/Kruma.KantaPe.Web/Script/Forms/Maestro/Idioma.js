...
var urlEdicion = 'RegistroIdioma.aspx';
...
function ListarIdiomaResultado(obj_pResultado, int_pNumPagina) {
    ListarIdioma(int_pNumPagina);
}

function ListarIdioma(int_pNumPagina) {
    int_NumPagina = int_pNumPagina;
    var obj_Filtro = ObtenerFiltros();
    var obj_Data = { "int_pIdIdioma": obj_Filtro.IdIdioma, "str_pDescripcion": obj_Filtro.Descripcion, "str_pEstado": obj_Filtro.Estado, "int_pNumPagina": int_pNumPagina, "int_pTamPagina": int_TamPagina };
    AccionDefault(true, "Idioma.aspx/ListarIdioma", obj_Data, CargarGrilla, null, null, null, 1);
}

function CargarGrilla(obj_pLista) {

    var obj_TablaPrincipal = $("<table id='tblIdioma' class='table table-bordered table-hover dt-responsive'></table>");
    ...
    for (var i = 0; i < obj_pLista.Result.length; i++) {
        ...
        obj_FilaPrincipal = $("<tr value='" + obj_pLista.Result[i].IdIdioma + "'></tr>");
    ...
    var obj_Tabla = $('#tblIdioma').dataTable({
        "language": {
...
function obtenerIdRegistro() {
    var str_Id = null;
    var obj_Seleccion = $('#tblIdioma').find('.selected');
    if (obj_Seleccion.length > 0)
        str_Id = $(obj_Seleccion[0]).attr('value');
    else
        AlertJQ(1, 'Seleccione un idioma.');
    return str_Id;
}
...
function Activar() {
    var str_Id = obtenerIdRegistro();
    if (str_Id != null) {
        ConfirmJQ('¿Está seguro de activar el idioma?', ModificarEstado, [str_Id, 'A']);
    }
}
...
function Inactivar(int_pId) {
    var str_Id = obtenerIdRegistro();
    if (str_Id != null) {
        ConfirmJQ('¿Está seguro de inactivar el idioma?', ModificarEstado, [str_Id, 'I']);
    }
}

function ModificarEstado(str_pId, str_pEstado) {
    Accion('Idioma.aspx/ModificarEstado', { "int_pIdIdioma": str_pId, "str_pEstado": str_pEstado }, ListarIdiomaResultado, [1]);
}
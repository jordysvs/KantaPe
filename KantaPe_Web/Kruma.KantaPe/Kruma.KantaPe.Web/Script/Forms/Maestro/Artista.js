...
function ListarArtistaResultado(obj_pResultado, int_pNumPagina) {
    ListarArtista(int_pNumPagina);
}

function ListarArtista(int_pNumPagina) {
    int_NumPagina = int_pNumPagina;
    var obj_Filtro = ObtenerFiltros();
    var obj_Data = { "int_pIdArtista": obj_Filtro.IdArtista, "str_pNombre": obj_Filtro.Nombre, "str_pEstado": obj_Filtro.Estado, "int_pNumPagina": int_pNumPagina, "int_pTamPagina": int_TamPagina };
    AccionDefault(true, "Artista.aspx/ListarArtista", obj_Data, CargarGrilla, null, null, null, 1);
}

function CargarGrilla(obj_pLista) {

    var obj_TablaPrincipal = $("<table id='tblArtista' class='table table-bordered table-hover dt-responsive'></table>");
    var obj_THead = $("<thead></thead>");
    var obj_FilaPrincipal = $("<tr></tr>");
    obj_FilaPrincipal.append($("<th style='width:85%'>Nombre</th>"));
    ...
    for (var i = 0; i < obj_pLista.Result.length; i++) {
        obj_FilaPrincipal = $("<tr value='" + obj_pLista.Result[i].IdArtista + "'></tr>");
    ...
    var obj_Tabla = $('#tblArtista').dataTable({
        "language": {
    ...
    $('#tblArtista tbody').on('click', 'tr', function () {
...
function obtenerIdRegistro() {
    var str_Id = null;
    var obj_Seleccion = $('#tblArtista').find('.selected');
...
function Activar() {
    var str_Id = obtenerIdRegistro();
    if (str_Id != null) {
        ConfirmJQ('¿Está seguro de activar el artista?', ModificarEstado, [str_Id, 'A']);
    }
}
...
function Inactivar(int_pId) {
    var str_Id = obtenerIdRegistro();
    if (str_Id != null) {
        ConfirmJQ('¿Está seguro de inactivar el artista?', ModificarEstado, [str_Id, 'I']);
    }
}

function ModificarEstado(str_pId, str_pEstado) {
    Accion('Artista.aspx/ModificarEstado', { "int_pIdArtista": str_pId, "str_pEstado": str_pEstado }, ListarArtistaResultado, [1]);
}
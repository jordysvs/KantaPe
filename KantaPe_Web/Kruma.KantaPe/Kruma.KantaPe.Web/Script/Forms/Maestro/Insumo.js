...
function ObtenerFiltros() {
    ...
    obj_Filtro.IdUnidadMedida = null;
    if ($("#ddlUnidadMedida").val() != '')
        obj_Filtro.IdUnidadMedida = $("#ddlUnidadMedida").val();

    obj_Filtro.Estado = $("#ddlEstado").val();
    return obj_Filtro;
}

function ListarInsumo(int_pNumPagina) {
    ...
    var obj_Data = {
        ...
        "int_pIdUnidadMedida": obj_Filtro.IdUnidadMedida,
        "str_pEstado": obj_Filtro.Estado,
        "int_pNumPagina": int_pNumPagina,
        "int_pTamPagina": int_TamPagina
    };
    AccionDefault(true, "Insumo.aspx/ListarInsumo", obj_Data, CargarGrilla, null, null, null, 1);
}

function CargarGrilla(obj_pLista) {

    var obj_TablaPrincipal = $("<table id='tblInsumo' class='table table-bordered table-hover dt-responsive'></table>");
    var obj_THead = $("<thead></thead>");
    var obj_FilaPrincipal = $("<tr></tr>");
    obj_FilaPrincipal.append($("<th style='width:20%;'>Código</th>"));
    obj_FilaPrincipal.append($("<th style='width:40%;'>Descripción</th>"));
    obj_FilaPrincipal.append($("<th style='width:25%;'>Unidad Medida</th>"));
    ...
    for (var i = 0; i < obj_pLista.Result.length; i++) {
        ...
        obj_FilaPrincipal = $("<tr value='" + obj_pLista.Result[i].IdInsumo + "'></tr>");
        obj_FilaPrincipal.append($("<td>" + obj_pLista.Result[i].Codigo + "</td>"));
        obj_FilaPrincipal.append($("<td>" + obj_pLista.Result[i].Descripcion + "</td>"));
        obj_FilaPrincipal.append($("<td>" + obj_pLista.Result[i].UnidadMedida.Descripcion + "</td>"));
    ...
    var obj_Tabla = $('#tblInsumo').dataTable({
        "language": {
    ...
    $('#tblInsumo tbody').on('click', 'tr', function () {
...
function obtenerIdRegistro() {
    var str_Id = null;
    var obj_Seleccion = $('#tblInsumo').find('.selected');
    if (obj_Seleccion.length > 0)
        str_Id = $(obj_Seleccion[0]).attr('value');
    else
        AlertJQ(1, 'Seleccione un insumo.');
    return str_Id;
}
...
function Activar() {
    var str_Id = obtenerIdRegistro();
    if (str_Id != null) {
        ConfirmJQ('¿Está seguro de activar el insumo?', ModificarEstado, [str_Id, 'A']);
    }
}
...
function Inactivar(int_pId) {
    var str_Id = obtenerIdRegistro();
    if (str_Id != null) {
        ConfirmJQ('¿Está seguro de inactivar el insumo?', ModificarEstado, [str_Id, 'I']);
    }
}

function ModificarEstado(str_pId, str_pEstado) {
    Accion('Insumo.aspx/ModificarEstado', {
        "int_pIdInsumo": str_pId,

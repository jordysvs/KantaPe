...
function CargarLocal(bln_pAsync, int_pIdLocal) {
    ...
    if (int_IdPersona != '') {
        ...
        AccionDefault(bln_pAsync, "RegistroProducto.aspx/ListarLocal", obj_Data,
             function (obj_pLista) {
...
function GuardarProducto() {
    ...
    if ($('#frmContent').valid()) {
        ...
        var str_Descripcion = $("#txtNombre").val().toLowerCase();
        //Tener en una cadena separado por espacios
        var str_Separador = str_Descripcion.split(" ");
        var str_NombreValidado = "";
        for (var i in str_Separador) {
            str_NombreValidado += str_Separador[i].substr(0, 1).toUpperCase() + str_Separador[i].substr(1) + " ";
        }

        obj_Producto.Nombre = str_NombreValidado;
...
function ObtenerProducto() {
    ...
    if (int_IdProducto != '') {
        ...
        if (obj_Producto != null) {
            ...
            CargarLocal(true, obj_Producto.IdLocal);
            ...
            if (obj_Producto.ImagenURL == '') {
                obj_Producto.ImagenURL = $("#hdProductoImagen").val();
                $("#lnkEliminarFoto").css('display', 'none');
            }

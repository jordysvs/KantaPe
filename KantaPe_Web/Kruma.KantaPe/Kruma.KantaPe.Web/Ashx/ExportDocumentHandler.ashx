...
public class ExportDocumentHandler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext obj_pContext)
    {
        ...
        if (obj_pContext.Request.Form.Count > 0)
        {
            ...
            if (!string.IsNullOrEmpty(obj_pContext.Request["bln_Worker"]))
                bln_pWorker = bool.Parse(obj_pContext.Request["bln_Worker"]);
            ...
            {
                if (bln_pWorker)
                    obj_Documento = Kruma.Core.Util.Export.HTMLToPDFWorker(str_HTML);
                else
